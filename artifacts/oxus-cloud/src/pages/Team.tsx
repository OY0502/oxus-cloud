...
import { useProjects, useTeamKpis, useTeamRoster } from "@/hooks/api";
...
import { OutstandingPayablesBreakdownDialog } from "@/components/team/OutstandingPayablesBreakdownDialog";
...
export function Team() {
  ...
  const [payablesBreakdownOpen, setPayablesBreakdownOpen] = useState(false);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <PageHeader
        title="Team"
        subtitle="Manage OXUS team members — roster, availability, projects, and workspace access."
      ...
      {kpis && (
        <div className={cn(
          ...
          isSuperAdmin ? "md:grid-cols-5" : "md:grid-cols-3",
        )}>
          <MetricCard
            title="Active team"
            value={String(kpis.active_team)}
            icon={<Users className={teamIcon} />}
            className="shadow-none"
            valueClassName="text-2xl font-serif"
          />
          <MetricCard
          ...
          <MetricCard
            ...
            value={kpis.has_capacity_data && kpis.fully_allocated != null ? String(kpis.fully_allocated) : "—"}
            className="shadow-none"
            valueClassName="text-2xl font-serif"
          />
          ...
          {isSuperAdmin && kpis.has_payable_data && (
            <MetricCard
              title="Ready to pay"
              value={formatEUR(kpis.ready_to_pay ?? 0)}
              className="shadow-none cursor-pointer"
              valueClassName="text-2xl font-serif"
              onClick={() => setPayablesBreakdownOpen(true)}
            />
          )}
          {isSuperAdmin && kpis.has_payable_data && (
            <MetricCard
              title="Outstanding payables"
              value={formatEUR(kpis.outstanding_payables ?? 0)}
              className="shadow-none cursor-pointer"
              valueClassName="text-2xl font-serif"
              onClick={() => setPayablesBreakdownOpen(true)}
            />
        ...
        <TabsContent value="roster" className="mt-5 space-y-4">
          ...
          {rosterQuery.isLoading ? (
            ...
            <EmptyState
              ...
              description="Add team members to track availability, projects, and compensation."
              action={isSuperAdmin ? (
      ...
      <OutstandingPayablesBreakdownDialog
        open={payablesBreakdownOpen}
        onOpenChange={setPayablesBreakdownOpen}
        onRecordPayment={(personId) => {
          const person = rosterQuery.data?.find((r) => r.person.id === personId)?.person;
          if (person) openDrawer(person, "payables");
        }}
      />
