...
import { readProjectsView, writeProjectsView } from "@/lib/ganttScale";
...
function ArchivedBadge() {
  ...
  return <Badge variant="secondary" className="text-[10px] uppercase">Archived</Badge>;
...
function ProjectCard({ p }: { p: ProjectWithAssignees }) {
  ...
  return (
    ...
    <Card className={cn("mb-3 hover-elevate", archived && "opacity-60")}>
      ...
      <CardContent className="p-4 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-foreground">{formatEUR(p.budget)}</span>
...
function readLifecycleFilter(search: string): LifecycleFilter {
...
export function Projects() {
  ...
  const [view, setView] = useState(() => readProjectsView() ?? "table");

  const [lifecycle, setLifecycle] = useState<LifecycleFilter>(() => readLifecycleFilter(search));
  ...
  useEffect(() => {
    ...
    setLifecycle(readLifecycleFilter(search));
  ...
  const filtered = useMemo(() => {
    ...
    if (lifecycle === "active") list = list.filter((p) => !p.archived_at);

    if (lifecycle === "archived") list = list.filter((p) => !!p.archived_at);
  ...
  }, [projects, lifecycle, query]);



  useEffect(() => { setBoardItems(filtered.filter((p) => !p.archived_at)); }, [filtered]);
  ...
  const setFilter = (next: LifecycleFilter) => {
    ...
    navigate(next === "active" ? "/projects" : `/projects?filter=${next}`);
      ...
      cell: (item: ProjectWithAssignees) => (
        ...
        <div className="flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {item.archived_at && <ArchivedBadge />}
  ...
  const editProject = (p: ProjectWithAssignees) => navigate(`/projects/${p.id}/edit`);
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        subtitle="Command center for delivery work. Archived projects stay available under Archived."
      ...
      <Tabs value={view} onValueChange={setView} className="w-full">
              ...
              {(["active", "archived", "all"] as const).map((f) => (
                ...
                <button
                  ...
                  className={cn(
                    ...
                    lifecycle === f ? "bg-background text-foreground shadow-sm" : "text-muted-foreground",
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            title={lifecycle === "archived" ? "No archived projects" : "No projects yet"}
            ...
            description={
              ...
              lifecycle === "archived"
                ...
                : "Spin up your first project to track delivery, timelines, and team workload."

            }

            action={lifecycle === "active" ? <Button onClick={() => navigate("/projects/new")}><Plus className="h-4 w-4 mr-2" />New Project</Button> : undefined}
          ...
          <>
            ...
            <TabsContent value="table" className="m-0 border-none p-0 outline-none">
              ...
              <DataTable
                ...
                getRowClassName={(item) => (item.archived_at ? "opacity-60" : undefined)}
            ...
            <TabsContent value="board" className="m-0 border-none p-0 outline-none">
              ...
              {lifecycle === "archived" ? (
                ...
                <EmptyState
                  ...
                  title="Board shows active projects"

                  description="Switch to Active or All to use the kanban board."
            ...
            <TabsContent value="timeline" className="m-0 border-none p-0 outline-none">
              ...
              <ProjectGantt projects={filtered} onOpen={openProject} onEdit={editProject} />
