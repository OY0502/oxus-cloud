...
export function detectDuplicateTask(args: {
  ...
  for (const task of args.existingTasks) {
    ...
    const proposedNorm = normalizeTitle(args.proposedTitle);
    if (titleNorm === proposedNorm) {
      return {
        is_duplicate: true,
        duplicate_candidate_id: task.id,
    ...
    if (score >= 0.72 && (!best || score > best.score)) {
      best = {
        task,
        score,
        reason: titleScore >= requestScore ? "High title similarity" : "Request matches existing task objective",
      };
    }
  }
...
export function assessTaskComplexity(request: string, namedTechnologies: string[] = []): "simple" | "moderate" | "complex" {
  ...
  if (
    namedTechnologies.length === 0 &&
    (/\b(change|update|fix|rename|replace)\b.*\b(text|label|copy|button|title)\b/.test(lower) ||
      lower.length < 80)
  ) {
    return "simple";
  }
  if (
    namedTechnologies.length >= 1 ||
    /\bintegrat(e|ion)\b|\bmigrat(e|ion)\b|\bmultilingual\b|\bi18n\b|\blocali[sz]e\b|\barchitect\b/.test(lower) ||
    lower.length > 200
  ) {
    return "complex";
  }
...
export function extractNamedTechnologies(text: string): string[] {
  ...
  const patterns: [string, RegExp][] = [
    ...
    ["PandaDoc", /\bpandadoc\b/],
    ["Mixpanel", /\bmixpanel\b/],
    ["TipTap", /\btiptap\b/],
    ["Localise", /\blocali[sz]e\b|\blokalise\b/],
  ];
