...
import { reprocessSlackEventsForProject } from "../_shared/reprocessSlackEvents.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let body: { project_id?: string; project_slack_link_id?: string; force?: boolean } = {};
    try {
      body = await req.json();
    } catch {
    ...
    const result = await reprocessSlackEventsForProject({
      admin,
      projectId,
    ...
    return err("Failed to reprocess Slack events.", 500, "REPROCESS_ERROR", (e as Error).message);
  }
});
