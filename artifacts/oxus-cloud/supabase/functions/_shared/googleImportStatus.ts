...
export type GoogleSyncStage =
  | "idle" | "queued" | "validating_connection" | "syncing_contacts" | "syncing_calendar" | "syncing_gmail"
  ...
  | "creating_candidates" | "enriching_companies" | "finalizing" | "completed" | "completed_with_warnings" | "failed" | "timed_out";

export type GoogleImportRun = {
  id: string;
...
function mapStage(run: GoogleImportRun, activeWork: boolean): GoogleSyncStage {
  ...
  const known = new Set<GoogleSyncStage>([
    ...
    "discovering_gmail_threads", "processing_gmail_threads", "resolving_basic_people", "resolving_basic_companies",
    "core_sync_complete", "filtering_relationship_threads", "analyzing_relationships", "enrichment_complete",
    "resolving_entities", "resolving_people", "resolving_companies", "processing_relationships",
...
export async function loadCanonicalGoogleImportStatus(
  admin: SupabaseClient,
  input: {
    connectionId: string;
    ownerUserId: string;
    run?: GoogleImportRun | null;
    connectionError?: string | null;
  },
): Promise<GoogleCanonicalImportStatus> {
  const run = input.run ?? await loadLatestImportRun(admin, input.connectionId);
  const sourceRuns = run ? await loadSourceRuns(admin, run.id) : [];
  const reviewCandidatesPending = await countPendingReviewCandidates(admin, input.ownerUserId, input.connectionId);
  return resolveCanonicalGoogleImportStatus({
    run,
    sourceRuns,
    reviewCandidatesPending,
    connectionError: input.connectionError ?? null,
  });
}

async function loadLatestImportRun(admin: SupabaseClient, connectionId: string): Promise<GoogleImportRun | null> {
  const { data } = await admin
    ...
    .limit(1)
    .maybeSingle();
  return (data as GoogleImportRun | null) ?? null;
}

async function loadSourceRuns(admin: SupabaseClient, importRunId: string): Promise<GoogleImportSourceRunSnapshot[]> {
  const { data } = await admin
    .from("google_import_source_runs")
    .select("source, status, current_stage, last_heartbeat_at")
    .eq("import_run_id", importRunId);
  return (data ?? []) as GoogleImportSourceRunSnapshot[];
}

async function countPendingReviewCandidates(
  admin: SupabaseClient,
  ownerUserId: string,
  connectionId: string,
): Promise<number> {
