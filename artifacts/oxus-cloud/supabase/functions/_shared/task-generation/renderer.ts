import type { TaskSpecification } from "./schema.ts";
...
export function renderTaskSpecificationMarkdown(spec: TaskSpecification): string {
  const lines: string[] = [];
  if (spec.objective?.trim()) lines.push("## Objective", spec.objective.trim(), "");
  const contextItems = [
    ...(spec.context ?? []),
  ...
  if (spec.assumptions?.length) lines.push(...bulletSection("## Assumptions", spec.assumptions));
  if (spec.blocking_questions?.length) lines.push(...bulletSection("## Blocking questions", spec.blocking_questions));
  if (spec.out_of_scope?.length) lines.push(...bulletSection("## Out of scope", spec.out_of_scope));
  if (spec.risks?.length) lines.push(...bulletSection("## Risks", spec.risks));
  return lines.join("\n").trim();
}
...
export function renderLegacyProposedTaskMarkdown(task: {
  ...
  const lines: string[] = [];
  if (task.description?.trim()) lines.push("## Objective", task.description.trim(), "");
  ...
  if (Array.isArray(task.qa_scenarios) && task.qa_scenarios.length > 0) {
    ...
    for (const scenario of task.qa_scenarios) {
      ...
      if (Array.isArray(scenario.steps)) for (const step of scenario.steps) lines.push(`- ${step}`);
